...
def receive(mailbox_addr, rcvr_sk, sndr_pk):
    ...
    for tx_id in tx_ids:
        if not tx_id in msg_tx_ids:
            ...
            if btc_d == -1:
                print('[WARN] Recovering private key failed for tx {tx_id}. Skipping.')
                continue
            #print(f'private key recovered: {sndr_sk}')
...
def contains_amsg(tx_hex, ecdh_res):
    ...
    #print(f'k_chat: {k_chat}\tr_chat: {r_chat}')
...
def transaction_hash(tx_hex, pubKey):
    tx = Transaction.import_raw(tx_hex, network='testnet')
